...
**Reference Application**: Support Chat Platform — `simple-express-backend` (Express/Socket.IO Backend)  
**Repository**: [https://github.com/sujanustc/ostad-capstan-backend](https://github.com/sujanustc/ostad-capstan-backend)  
**Frontend Repository**: [https://github.com/sujanustc/ostad-capstan-frontend](https://github.com/sujanustc/ostad-capstan-frontend)  
...
### 1. Production Backend API (Express + Socket.IO)
![Backend API Response](docs/images/backend_api.png)
*Figure 1: Live Backend API configuration endpoint (`http://13.203.161.35:30021/api/config/chat`) returning HTTP 200 JSON payload.*

### 2. Production Frontend UI
![Frontend UI Demonstration](docs/images/frontend_app.png)
*Figure 2: Live Support Chat React/Vite Frontend running on Kubernetes Production (`http://13.203.161.35:30022`).*
...
* **Dev & Stage Environments (Manual Trigger)**:
  * Pushing code to `dev` or `stage` automatically triggers CI linting, type-checking, building, and pushing Docker images (`dasujandb/support-chat-backend:${ENV}-${SHA}-${TIMESTAMP}`).
...
* **Production Environment (Automatic Trigger on PR Open)**:
  ...
  * The pipeline builds optimized production containers, updates the Kubernetes deployment manifest image tags in `k8s/prod/backend.yaml`, and commits back to Git.
...
Two distinct Dockerfiles were engineered for the backend:

* **Development Image (`Dockerfile.dev`)**:
  * Runs TypeScript TSX watch execution (`NODE_ENV=development`).
  * Includes full devDependencies for rapid developer debugging.

* **Production Image (`Dockerfile.prod`)**:
  * Multi-stage build process.
  * **Stage 1**: Compiles TypeScript into JavaScript (`npm run build`).
  * **Stage 2**: Installs production-only dependencies (`npm install --omit=dev`) and runs under non-root user `node` (`NODE_ENV=production`).

* **Traceable Versioning Scheme**:
  * Format: `${ENVIRONMENT}-${GIT_COMMIT_SHA_SHORT}-${BUILD_TIMESTAMP}`
  * Example Tag: `prod-4b8c9e3-202608270208`
...
* **Networking (No Ingress)**:
  ...
  * **Port Mapping**:
    * `dev`: Backend NodePort `30001` (Frontend NodePort `30002`)
    * `stage`: Backend NodePort `30011` (Frontend NodePort `30012`)
    * `prod`: Backend NodePort `30021` (Frontend NodePort `30022`)
...
* **Sync Policies**:
  * `support-chat-backend-dev` & `stage`: `Manual` Sync Policy.
  * `support-chat-backend-prod`: `Automated` Sync Policy (`prune: true`, `selfHeal: true`).
